// src/events/interactions/buttons/button-grupales.js - Botones para eventos de Zona Abierta
...
export default async function (interaction) {
    ...
    const action = prefixRaw.replace("grupales-", "");
    ...
    if (action === "salir") {
        ...
        const faccionEmoji = FACCIONES[event.faccion.toUpperCase()] || "";
        ...
        // Truncar si es muy largo (límite de Discord: 1024 caracteres)
        const truncatedList = list.length > 900 ? list.substring(0, 900) + "..." : list;

        const embed = EmbedBuilder.from(interaction.message.embeds[0])
            .setFields(
                { name: "📢 CALLER", value: `<@${event.creatorId}>`, inline: true },
                { name: "🏷️ FACCION", value: `${faccionEmoji} ${event.faccion.toUpperCase()}`, inline: true },
        ...
        log.interaction('button', `grupales-salir`);
    ...
    if (action === "agregar") {
        ...
        const modal = new ModalBuilder()
            .setCustomId(`grupales-agregar-modal|${eventId}`)
        ...
        const roleSelect = new TextInputBuilder()
            ...
            .setLabel("Rol (tank/heal/flami/sc/badon/dps)")
            .setStyle(TextInputStyle.Short)
            .setPlaceholder("Escribe el rol: tank, heal, flami, sc, badon, dps")
            .setRequired(true);
    ...
    if (action === "quitar") {
        ...
        const modal = new ModalBuilder()
            .setCustomId(`grupales-quitar-modal|${eventId}`)
        ...
        try {
            ...
            if (logChannel) {
                ...
                const reportEmbed = new EmbedBuilder()
                    .setTitle(`👑 EVENTO FINALIZADO: ${event.titulo.toUpperCase()}`)
                    .setColor(COLORS.grupales)
                    .addFields(
                        { name: "FACCION", value: `${FACCIONES[event.faccion.toUpperCase()]} ${event.faccion.toUpperCase()}`, inline: true },
                        { name: "CALLER", value: `<@${event.creatorId}>`, inline: true },
                        { name: "HORA", value: event.hora, inline: true },
            ...
            log.error("Error en reporte grupales:", err);
        }
